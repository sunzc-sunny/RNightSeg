...
def main():
    args = parse_args()
    # args.config = 'configs/deeplabv3plus/deeplabv3plus_r101_fd.py'
    # args.gpu_id = 4
    ...
    # 判断cfg中是否有ts配置
    if cfg.get('ts', None) is not None:

        print("using train strategy")
        model = build_train_model(
            cfg, train_cfg=cfg.get('train_cfg'), test_cfg=cfg.get('test_cfg'))
    else:
        print('No train strategy is specified')
