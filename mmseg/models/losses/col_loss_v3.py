...
class ColorProcessingLossV3(nn.Module):
    def __init__(self,
                 loss_weight=1,
                 loss_name='loss_col_v3',
                 avg_non_ignore=False
                 ):
        super(ColorProcessingLossV3, self).__init__()
    ...
    def forward(self, reflectance, org_img, seg_label):
        ...
        new_illumination = illumination.clone()
        # illumination = torch.clamp(illumination, 0, 1)
        new_illumination = self.gamma(new_illumination, 0.4)
        new_illumination[seg_label == 10] = illumination[seg_label == 10]
