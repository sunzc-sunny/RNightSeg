...
class ColorProcessingLoss(nn.Module):
    def __init__(self,
                 loss_weight=1,
                 loss_name='loss_col',
                 avg_non_ignore=False
                 ):
        super(ColorProcessingLoss, self).__init__()
    ...
    def forward(self, reflectance, org_img, seg_label):
        ...
        # print("illumination_org", illumination.max().item(), illumination.min().item())

        for i in range(19):
            illumination[seg_label == i] = illumination[seg_label == i]/(self.illumination_class_nightcity[i]/255)
        # illumination = torch.clamp(illumination, 0, 1)
        illumination = self.gamma(illumination, 0.4)
        # print("illumination_gama", illumination.max().item(), illumination.min().item())
        for i in range(19):
            illumination[seg_label == i] = illumination[seg_label == i]*(self.illumination_class_cityscape[i]/255)
