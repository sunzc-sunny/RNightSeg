...
def HL(loss, weight):
    ...
    loss_detach = loss.clone().detach()
    weight_detach = weight.clone().detach()
    loss = torch.sum(loss * weight_detach)
    weight_loss = -0.01 * torch.sum(loss_detach * weight)
...
class SegformerParallelMaskHead(BaseDecodeHead):
    def __init__(self, interpolate_mode='bilinear', **kwargs):
        ...
        self.fuse_layer_2 = nn.Conv2d(in_channels=2*self.channels * num_inputs, out_channels=self.channels * num_inputs, kernel_size=1, stride=1, padding=0)

        # self.sigmoid = nn.Sigmoid()
        self.DualAtt_ConBlock = DualAtt_ConBlock(inchannels=self.channels * num_inputs, outchannels=self.channels * num_inputs)
        self.local_iter = 0
    ...
    def forward(self, feature):
        ...
        dual_att_input = self.fuse_layer_1(torch.cat([reflectance_clone, output_clone], dim=1))

        dual_att_output = self.DualAtt_ConBlock(dual_att_input)

        outs = self.fuse_layer_2(torch.cat([dual_att_output, outs], dim=1))
    ...
    def losses(self, seg_logits, seg_labels, img_metas):
        ...
        if isinstance(seg_logits, tuple):
            ...
            if self.local_iter > 40000:
                ...
                topk = (topk,)
                maxk = max(topk)
                pred_value, pred_label = seg_logit.topk(maxk, dim=1)

                # seg_label[seg_label == 255] = pred_label[seg_label == 255]
                confidence_map = torch.abs(pred_label - seg_label)
                confidence_map[confidence_map > 0] = 1
                confidence_map = 2-confidence_map
                # confidence_map = confidence_map - 5
                # for m in [6, 7, 11, 12, 13, 14, 15, 16, 17, 18]:
                #     confidence_map[seg_label == m] += 5
                # for m in [0, 1, 2, 3, 4, 5, 8, 9, 10]:
                #     confidence_map[seg_label == m] += 4

                seg_weight = confidence_map.squeeze(1)
            self.local_iter += 1
            dev = org_img.device
