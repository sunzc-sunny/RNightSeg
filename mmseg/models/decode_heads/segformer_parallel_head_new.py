...
class SegformerParallelHeadNew(BaseDecodeHead):
    def __init__(self, interpolate_mode='bilinear', **kwargs):
        ...
        self.fuse_layer_2 = nn.Conv2d(in_channels=2 * self.channels * num_inputs, out_channels=self.channels * num_inputs,
    ...
    def forward(self, feature):
        ...
        reflectance_output = resize(reflectance_output, size=img.shape[2:], mode='bilinear',
                                    align_corners=self.align_corners)

        dual_att_input = torch.cat([reflectance_clone, output_clone], dim=1)
        dual_att_input = self.fuse_layer_1(dual_att_input)
