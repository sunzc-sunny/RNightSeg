...
class ParallelRefASPPModel(DepthwiseSeparableASPPHead):
    def __init__(self, **kwargs):
        super(ParallelRefASPPModel, self).__init__(**kwargs)
        ...
        #         norm_cfg=self.norm_cfg,
        #         act_cfg=self.act_cfg))
        self.reflectance_body = DepthwiseSeparableConvModule(
                self.channels + self.c1_channels,
                self.channels,
                3,
                padding=1,
        ...
        self.reflectance_bottleneck = DepthwiseSeparableConvModule(
                ...
                act_cfg=self.act_cfg)
        # 一个1x1的conv
        self.fuse_layer = nn.Conv2d(in_channels= self.channels+self.channels+self.c1_channels, out_channels= self.channels+self.c1_channels, kernel_size=1, stride=1, padding=0)
    ...
    def forward_test(self, inputs, img_metas, test_cfg):
        ...
        d_value = torch.tensor([9.949114786568614
            ...
            , -28.018538355823964
            , 11.451970012384827
            , 14.875401536546029, 197.3442703678064, 9.4808833704288, -1.8101781201072953, -14.062115432293666, -17.00952531398586, -18.254379668453765, 29.860365832960653, -5.757649588478117, 6.946705122880488])
        ...
        if self.local_iter % 1000 == 0:
            ...
            for i in range(19):
                illumination[pred_label == i] = illumination[pred_label == i]/(illumination_class_nightcity[i] / 255)

            new_illumination = self.gamma(illumination, 0.5)
            for i in range(19):
                new_illumination[pred_label == i] = new_illumination[pred_label == i]*(illumination_class_cityscape[i] / 255)

            bright_img = vis_ref * new_illumination

            vis_bright = torch.clamp(bright_img, 0, 1)

            out_dir = './img_bright_new12'
            ...
            for j in range(batchsize):
                ...
                plt.savefig(os.path.join(out_dir, f'{self.local_iter}_{j}.png'))
                plt.close()
    ...
    def forward(self, inputs):
        ...
        reflectance = self.reflectance_body(output_clone)
        ...
        # """"""""""""""""""""""""""" #
        output = self.fuse_layer(torch.cat([output, reflectance_clone], dim=1))
