...
class ParallelASPPModelSigmoid(DepthwiseSeparableASPPHead):
    def __init__(self, **kwargs):
        super(ParallelASPPModelSigmoid, self).__init__(**kwargs)
    ...
    def forward(self, inputs):
        ...
        reflectance = self.reflectance_bottleneck(output_clone)

        # print(output.shape)
        # print(reflectance.shape)
        # print(torch.cat([output, reflectance], dim=1).shape)
        # 使用self.fuse_layer 融合output和reflectance
        output = self.fuse_layer(torch.cat([output, reflectance], dim=1))
